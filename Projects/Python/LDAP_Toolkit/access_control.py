...
class Ldap:
    ...
    def modify_entry(self, entry, action):
        if action == 'add':
            cmd = f'/usr/bin/ldapadd -v -x -D "{self.credentials["bind_dn"]}" -w "{self.credentials["password"]}"'
        elif action == 'modify':
            cmd = f'/usr/bin/ldapmodify -v -x -D "{self.credentials["bind_dn"]}" -w "{self.credentials["password"]}"'
        elif action == 'delete':
            cmd = f'/usr/bin/ldapdelete -v -D "{self.credentials["bind_dn"]}" -w "{self.credentials["password"]}"'
...
class LdapData(Ldap):
    ...
    def set_user_info(self):
        self.first_name = terminal.get_input(message=f'First Name', message_option='blue')
        self.last_name = terminal.get_input(message=f'Last Name', message_option='blue')
    ...
    def set_sudo_level(self, single=True):
        ...
        while True:
            if not single:
                terminal.print_info(f'EXAMPLE: If you specify {terminal.return_format(self.valid_sudo_levels[-1], "red", end="blue")}, sudo levels {terminal.return_format(self.valid_sudo_levels[0], "red", end="blue")}-{terminal.return_format(self.valid_sudo_levels[-1], "red", end="blue")} would be added or removed.', start='\n')
            sudo_level = terminal.get_input(message=f'Sudo level ({terminal.return_format(self.valid_sudo_levels[0], "red", end="blue")}-{terminal.return_format(self.valid_sudo_levels[-1], "red", end="blue")})', message_option='blue')
            if single:
    ...
    def set_action(self):
        while True:
            action = terminal.get_input(message=f'Do you want to add or remove user {terminal.return_format(self.username, "yellow", end="blue")}? ({terminal.return_format("add", "red", end="blue")}/{terminal.return_format("del", "red", end="blue")})', message_option='blue')
...
class Jira:
    ...
    def set_ticket_number(self):
        while True:
            ticket_number = terminal.get_input(message=f'Which JIRA ticket is this request assocaited with? (Example: {terminal.return_format("INFRA-1234", "red", end="blue")})', message_option='blue')
    ...
    def get_ticket_info(self, ticket_number):
        jira_url = f'https://jira.sigfig.com/rest/api/latest/search?jql=key%20%3D%20{ticket_number}'
        try:
            jira_request = requests.get(jira_url, headers=self.headers)
    ...
    def add_ticket_comment(self, actions):
        ...
        jira_url = f'https://jira.sigfig.com/rest/api/latest/issue/{self.ticket_number}/comment'
        try:
            add_comment = requests.post(jira_url, data=json.dumps(comment), headers=self.headers)
    ...
    if manage_kerberos:
        ...
        for principal in new_principals:
            if principal not in old_principals:
                kadmin.modify_principal(principal, 'add')
...
def modify_sudo_command(ldap, target_sudo_command=None):
    ...
    ldap.identifiers = f'ou=level6,ou=SUDOers,dc=sigfig,dc=com'
...
if __name__ == '__main__':
    ...
    run_screen.start()
    ldap_password_file = '/root/sysadmin/passwords/ldap_kerberos'
    jira_password_file = '/root/sysadmin/passwords/automation_user_jira.yml'
